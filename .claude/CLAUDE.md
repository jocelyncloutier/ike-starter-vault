...
Map notes live in `Atlas/` root. They are navigation notes that collect links to related notes, sources, and external resources about a topic.

**Naming convention:** All map notes end with "Map" — e.g., "Product Strategy Map", "Home Network Map". This makes maps instantly identifiable in Cmd-O search.

**Structure:** Use topic-specific section headings, not generic ones. Start with `## Context` explaining what the map covers, then add sections that reflect how the notes group naturally. Do not use empty placeholder sections like "Related Maps" or "Open Questions" — only add sections that have content.

**Frontmatter:** Use `authorship: ai-generated` (or `ai-assisted`). Do not include `map` property — map notes don't belong to other maps.

## Weekly Reviews

Weekly reviews live in `Calendar/Weekly/`. They summarize the week's work and surface uncompleted tasks. The `## Week Overview` section is a human-written summary. The `## This Week's Tasks` and `## Carried Forward` sections are auto-generated by the Tasks plugin — do not manually populate them. Claude can help draft the Week Overview by scanning the week's daily notes.

## Monthly Reviews

Monthly reviews live in `Calendar/Monthly/`. They check in on efforts and summarize the month. The `## Effort Check-In` section uses the vault's folder-based effort statuses: Active (`Efforts/`), Later (`Efforts/Later/`), Paused (`Efforts/Paused/`). Claude can pre-fill this by scanning effort notes and the month's weekly reviews. The `## This Month's Tasks` section is auto-generated by the Tasks plugin.
